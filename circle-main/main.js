...
function draw() {
  ...
  if (ptInCIrcle(mouseX, mouseY, 350, 275, 40)) {
    document.body.style.backgroundColor = "blue";
  }
  if (ptInCIrcle(mouseX, mouseY, 475, 125, 60)) {
...
function ptInCIrcle(x1, y1, x, y, r) {
  let run = Math.abs(x1 - x);
  let rise = Math.abs(y1 - y);
  let dist = Math.sqrt(rise ** 2 + run ** 2);
  if (dist <= r) {
    return true;
  } else {
    return false;
